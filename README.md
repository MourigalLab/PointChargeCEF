...
5. The delta function above is convoluted to a Voigt function wherein the Gaussian function is used to account for the  energy-resolution of neutron spectrometer (refered as 'FWHM' below) and the Lorentzian function to represents the intrinsic broadening (or finite lifetime) of CEF excitations.  We are now using the resolution for SEQUOIA spectrometer at SNS, Oak Ridge National Lab, if you want to change the instrument resolution, one need to re-define the instrumental resolution function `PointchargeCEF.Instrument_resolution(x, Ei)`
